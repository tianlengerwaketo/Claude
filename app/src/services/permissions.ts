 ...
 *
 * - Android 12+ (API 31+): BLUETOOTH_SCAN or BLUETOOTH_ADVERTISE (whichever
 *   the requested role needs), requested with the "neverForLocation" flag on
 *   the native side (see android/app/src/main/AndroidManifest.xml) so we
 *   don't have to ask for location at all - we only use BLE, never GPS.
 *   Requested separately per role so a problem granting one (e.g. a
 *   permission missing from the manifest) doesn't also block the other -
 *   scanning and advertising are independent features in this app.
...
export type BleRole = 'advertise' | 'scan';
